...
class LibraAgent(AgentWrapper): 
    class GCCState(Enum):
        UNDERUSING = auto()
        OVERUSING = auto()
    class WLibraState(Enum): # Enum枚举类 https://blog.csdn.net/ProQianXiao/article/details/113481092
    # Exploration = 0
    # Evaluation = 1
    # Exploitation = 2
        Ordinary = 0
        EI_c_1 = 1 #在EI中使用第一阶段cl算出来的速率
        EI_r_1 = 2 #在EI中使用第一阶段rl算出来的速率
        EI_c_2 = 3 #receive ack of the class action and generate the reward 
        EI_r_2 = 4 #receive ack of the rl action and generate the reward
    ...
    def __init__(self, time_interval, base_dir) -> None:
        ...
        #---------------Libra init--------------------
        self.w_state = self.WLibraState.Ordinary # 当前的状态，Ordinary是指第一阶段
        self.EI_sequence = 0
        self.u_1 = 0 # utility value
        self.u_2 = 0
        self.u_3 = 0
        self.cwnd_list = []
        self.start_up = True
        ...
        self.prev_sending_rate = 100000 # 指论文中的Xprev
        self.latest_action = self.latest_bandwidth
        self.a_final = 0 # 当前的发送速率
        self.x_r = 0 # 指论文中的rl对应的cwnd
        self.x_c = 0 # 指论文中的cl对应的cwnd
    ...
    def get_estimated_bandwidth(self)->tuple:
        ...
        #增加一个排干逻辑
        ...
        elif self.w_state == self.WLibraState.Ordinary: # 当前处在第一阶段
            self.cwnd_list.clear()
            self.s1_rec_buffer = np.concatenate((self.s0_rec_buffer[self.single_s_dim:], self.s1) )
            # 使用agent，将agent的动作用a_rl进行记录，最终得到rl对应的cwnd值x_r
            a_rl = self.agent.get_action(self.s1_rec_buffer,False) #if i%200!=1 else agent.get_action(s0_rec_buffer,False);
            a_rl = np.squeeze(a_rl)
            ...
            if np.abs(self.x_r-self.x_c)>=0.01*self.prev_sending_rate: # prev_cwnd not defined yet
                ...
                if self.x_r>self.x_c:
                    ...
                    self.cwnd_list.append(self.x_c)  # cwnd_list[0] 小流；cwnd_list[1] 大流；
                    self.cwnd_list.append(self.x_r)
                    self.EI_sequence=1 # cl在先，EI_sequence设置为 1
                    ...
                    self.EI_sequence=0 # rl在先，EI_sequence设置为 0
                    self.a_final=self.cwnd_list[0]
                    self.w_state=self.WLibraState.EI_r_1
                    
            else:
                self.a_final=self.x_c # 如果仍在第一阶段的话，使用的是传统算法的速率
            ...
            # 进入这里说明两个EI全部已经完成，需要进入第三阶段进行utility value的计算
            # 在这个分支内，首先计算的是U(cl)
            else:      
                # 这里得到的是u(prev)
