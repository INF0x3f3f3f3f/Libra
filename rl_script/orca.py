...
def train(is_train_mode, report_interval_ms, train_mode):
    ...
    if not os.path.exists(pr_base): # 看看有没有/performance_records/路径，如果没有的话生成一个
        os.mkdir(pr_base)
    ...
    # %M 分钟数（00=59）
    # %S 秒（00-59）
    os.makedirs(log_dir_base, exist_ok=True) # 用于递归创建目录
    ...
    max_episodes = 200000
    update_interval = 900000 # 第一阶段时间长度的threshold
    save_interval = 100
    log_interval = 30
    reward_ewma_coef = 0.9 # ewma指数加权平均
    ewma_reward_record = []
    ewma_r = 0
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        if not Train:
            max_episodes = len(trace_set) # 由trace的长度决定episode的长度
        ...
        for i in range(1, max_episodes + 1):
            ...
            terminal = False # 比较两者的差值是否大于threshold，判断是否跳出第一阶段的bool值
            time_step = 0

            # 当两者的速率差值不大并且该阶段不长时，继续在第一个阶段
            while not terminal and time_step < update_interval: 
                ...
                a = agent.get_action(
                    s0_rec_buffer) if not logger_enabled else agent.get_action(
                        s0_rec_buffer, False)
                a = np.squeeze(a) # 从数组的形状中删除单维度条目，即把shape中为1的维度去掉
