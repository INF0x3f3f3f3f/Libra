...
class Delay_gradient_calculator: # 计算时延梯度，也就是utility function中的d(rtt)/d(t)
    ...
    def push(self, data: Tuple[int, int]) -> None: # 将新的时间戳值压入
        # if len(self.timestamps) > self.size:
        #     self.timestamps = self.timestamps[1:]
        self.timestamps.append(data)

    def get_gradient(self) -> np.float64:
        if len(self.timestamps) < 2: # 如果长度为1，返回0
            return 0.0
        ts = self.timestamps[:self.size] # https://www.cnblogs.com/RB26DETT/p/10971153.html [0,self.size)
        gradients = []
        for i in range(1, len(ts)):
            s = ts[i][0] - ts[i - 1][0]
            r = ts[i][1] - ts[i - 1][1]
            # (rec后 - rec前) - (send后 - send前) == (rec后 - send后) - (rec前 - send前) == rtt后 - rtt前
